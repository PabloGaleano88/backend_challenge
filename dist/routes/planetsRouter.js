    ...
    try {
        if (req.query) {
            ...
            if (typeof (name) === "string") {
                const info = yield (0, planetsController_1.getplanet)(name, limitQuery, pageQuery, sortQuery);
                res.send(info);
            }
            else {
                const info = yield (0, planetsController_1.getplanet)(null, limitQuery, pageQuery, sortQuery);
                res.send(info);
            }
        }
    }
...
planetsRouter.get("/:id", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        if (req.params) {
            const id = req.params.id;
            const planet = yield (0, planetsController_1.getPlanetById)(id);
            res.send(planet);
        }
    }
    catch (e) {
